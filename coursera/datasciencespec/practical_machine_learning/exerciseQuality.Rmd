...
### kNN model
Looking into unlabeled test set we can see that all the records describe the identical set of individuals as the training set so we can learn using the user anthropometric specifics. kNN seems as good candidate for using these specifics. Prior to that, we normalize the data, removing irrelevant *user_name*. Also, as we will use the cross-validation, we shuffle the rows in training set to break possible bias in folds (Fig 2.).
```{r normalization and shuffling}
N=length(names(mov1.filtered))
predObj=preProcess(trainMov[,-c(1,N)],method=c("center","scale"));

trainMov.stand=predict(predObj,trainMov[,-c(1,N)])
testMov.stand=predict(predObj,testMov[,-c(1,N)])

set.seed(222)
shuffle1=sample(nrow(trainMov.stand))
```
...
mod.knn=train(trainMov$classe[shuffle1]~.,data=trainMov.stand[shuffle1,],method="knn",trControl=ctrl,tuneGrid = expand.grid(.k=c(1,3,5,7,9)))
stopCluster(cl)
mod.knn
```
...
cl <- makeCluster(detectCores())
registerDoParallel(cl)
mod.svm=train(trainMov$classe[shuffle1]~.,data=trainMov.stand[shuffle1,],method="svmRadial",trControl=ctrl,tuneGrid = expand.grid(.sigma=c(0.01,0.02),.C=c(1,100,200)))
stopCluster(cl)
mod.svm
```
...
### Final choice
Due to very good performance, we choose SVM model. This model is expected to be very good classifier for **existing set of users** even in future. On the other hand, the trivial classifier is very probably to achieve the best results on the unlabelled test set, but it does not generalize to any new measurements.

We use the previously kept-out labelled test set to estimate the expected out-of-sample accuracy of the selected SVM model and we get the estimate **`r round(accuracy(predict(mod.svm,newdata=testMov.stand),testMov$classe),digits=6)`**.
...
cl <- makeCluster(detectCores())
registerDoParallel(cl)
mod.svm2=train(mov1.filtered$classe~.,data=mov1.fstand,method="svmRadial",tuneGrid = expand.grid(.sigma=0.02,.C=200))
stopCluster(cl)
```
...
```
### Conclusion
We intended to make a good classifier for exercise execution quality. After trying out several models, we chose SVM model based on crossvalidation estimated accuracy. We used obtained estimate of out-of-sample accuracy of **`r round(accuracy(predict(mod.svm,newdata=testMov.stand),testMov$classe),digits=6)`** using the labelled test set which signifies that the selected model will be very good classifier in the future for the users from the dataset, or the people of similar anthropometric characteristics. The accuracy will undoubtably be smaller for people that are not like the measured users.
